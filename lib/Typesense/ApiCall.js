...
class ApiCall {
    ...
    async get(endpoint, queryParameters = {}, { abortSignal = null, responseType = undefined, streamConfig = undefined, isStreamingRequest, } = {}) {
    ...
    async post(endpoint, bodyParameters = {}, queryParameters = {}, additionalHeaders = {}, { abortSignal = null, responseType = undefined, streamConfig = undefined, isStreamingRequest, } = {}) {
    ...
    async performRequest(requestType, endpoint, { queryParameters = null, bodyParameters = null, additionalHeaders = {}, abortSignal = null, responseType = undefined, skipConnectionTimeout = false, enableKeepAlive = undefined, streamConfig = undefined, isStreamingRequest, }) {
        var _a, _b, _c, _d;
    ...
    async handleStreamingResponse(response, streamConfig) {
        this.logger.debug(`Handling streaming response. Environment: ${isNodeJSEnvironment ? "Node.js" : "Browser"}`);
        ...
        return new Promise(async (resolve, reject) => {
            try {
                if (response.data && typeof response.data.getReader === "function") {
                    return this.handleBrowserReadableStream(response.data, resolve, reject, response, streamConfig);
                }
    ...
    combineMessageChunks(chunks, messagesChunks) {
        this.logger.debug(`Found ${messagesChunks.length} message chunks to combine`);
        const lastChunk = chunks[chunks.length - 1];
        if (this.isCompleteSearchResponse(lastChunk)) {
            return lastChunk;
        }
